...
class woo_instance_general_config_installer(models.TransientModel):
    ...
    order_import_next_execution = fields.Datetime('Next Execution2', help='Next execution time')
    order_import_user_id = fields.Many2one('res.users',string="User",help='User2',default=lambda self: self.env.user)
    
    
    order_auto_update=fields.Boolean(string="Auto Order Update ?",help="Check isf you want to automatically update order status to WooCommerce.")
    order_update_interval_number = fields.Integer('Update Order Interval Number2',help="Repeat every x.")
    order_update_interval_type = fields.Selection( [('minutes', 'Minutes'),
            ('hours','Hours'), ('work_days','Work Days'), ('days', 'Days'),('weeks', 'Weeks'), ('months', 'Months')], 'Update Order Interval Unit2')               
    order_update_next_execution = fields.Datetime('Next Executions', help='Next execution time')
    order_update_user_id = fields.Many2one('res.users',string="Users",help='User',default=lambda self: self.env.user)
    ...
    update_stock_interval_number = fields.Integer('Update Order Interval Number1',help="Repeat every x.")
    update_stock_interval_type = fields.Selection( [('minutes', 'Minutes'),
            ('hours','Hours'), ('work_days','Work Days'), ('days', 'Days'),('weeks', 'Weeks'), ('months', 'Months')], 'Update Order Interval Unit1')
    update_stock_next_execution = fields.Datetime('Next Execution1', help='Next execution time')
    update_stock_user_id = fields.Many2one('res.users',string="User1",help='User',default=lambda self: self.env.user)
