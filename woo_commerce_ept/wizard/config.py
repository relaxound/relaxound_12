...
class woo_instance_config(models.TransientModel):
    ...
    _description='res.config.woo.instance'
...
class woo_config_settings(models.TransientModel):
    ...
    order_update_next_execution = fields.Datetime('Next Execution1', help='Next execution time')    
    order_update_user_id = fields.Many2one('res.users',string="User1",help='User',default=lambda self: self.env.user)
    ...
    update_stock_user_id = fields.Many2one('res.users',string="user2",help='User',default=lambda self: self.env.user)
