...
class sale_order(models.Model):
    ...
    @api.multi
    def create_or_update_woo_customer(self, woo_cust_id, vals, is_company=False, parent_id=False, type=False,
                                      instance=False):
        ...
        if not partner:
            partner = partner_obj.search(
                [('name', '=', name), ('city', '=', city), ('street', '=', address1), ('street2', '=', address2),
                 ('email', '=', email), ('phone', '=', phone), ('zip', '=', zip), ('country_id', '=', country.id),
        ...
        if partner:
            ...
            partner = partner_obj.create(
                {'type': type, 'parent_id': parent_id, 'woo_customer_id': woo_customer_id or '',
    ...
    def get_woo_tax_id_ept(self, instance, tax_datas, tax_included):
        ...
        for tax in tax_datas:
            ...
            if rate != 0.0:
                ...
                if not acctax_id:
                    acctax_id = self.createWooAccountTax(rate, tax_included, instance.warehouse_id.company_id,
                                                         tax.get('name'))
                    if acctax_id:
        ...
        for line in lines:
            ...
            if line_product_id:
                woo_variant = woo_product_obj.search(
                    [('variant_id', '=', line_product_id), ('woo_instance_id', '=', instance.id)], limit=1)
                if woo_variant:
                    continue
                try:
                    if line_product_id:
                        ...
                        try:
                            ...
                            transaction_log_obj.create({
                                                           'message': "Json Error : While import Product with id %s from WooCommerce for instance %s. \n%s" % (
                                                           line_product_id, instance.name, e),
                                                           'mismatch_details': True,
                                                           'type': 'sales',
                                                           'woo_instance_id': instance.id
                                                           })
                    ...
                    woo_variant = False
                    message = "Variant Id %s not found in woo || default_code %s || order ref %s" % (
                    line_product_id, line.get('sku'), order_number)
                ...
                if woo_variant:
                    ...
                    barcode = 0
            sku = line.get('sku') or ''
            woo_variant = barcode and woo_product_obj.search(
                [('product_id.default_code', '=', barcode), ('woo_instance_id', '=', instance.id)], limit=1)
            if not woo_variant:
            ...
            if not odoo_product and not woo_variant:
                woo_variant = sku and woo_product_obj.search(
    ...
    @api.model
    def create_woo_sale_order_line(self, line, tax_ids, product, quantity, fiscal_position, partner, pricelist_id, name,
                                   order, price, is_shipping=False):
            ...
            so_line_vals.update(
                {
                    ...
                    'single_unit': quantity * 20,
                }
            )
        elif '80x' in name or '80X' in name:
            so_line_vals.update(
                {
                    'order_id': order.id,
                    'product_uom_qty': quantity,
    ...
    def create_or_update_woo_product(self, line, instance, wcapi):
        ...
        if variant_id:
            woo_product = woo_product_obj.search(
                [('woo_instance_id', '=', instance.id), ('variant_id', '=', variant_id)], limit=1)
            if woo_product:
                return woo_product
            woo_product = woo_product_obj.search(
                [('woo_instance_id', '=', instance.id), ('default_code', '=', line.get('sku'))], limit=1)
            ...
            for order_status in instance.import_order_status_ids:
                ...
                if int(total_pages) >= 2:
                    ...
                    order_ids = order_ids + self.import_all_woo_orders(wcapi, instance, transaction_log_obj,
                                                                       order_status, list3[0])

            import_order_ids = []

            for order in order_ids:
                if self.search([('woo_instance_id', '=', instance.id), ('woo_order_id', '=', order.get('id')),
                    ...
                    for tax in line.get('taxes'):
                        ...
                        account_tax = self.env['account.tax'].search(
                            [('price_include', '=', tax_included), ('type_tax_use', '=', 'sale'),
                             ('amount', '=', tax_rate), ('company_id', '=', instance.warehouse_id.company_id.id)],
                            limit=1)
    ...
    def update_woo_order_status(self, instance):
        ...
        if not instance:
            instances = self.env['woo.instance.ept'].search(
                [('order_auto_update', '=', True), ('state', '=', 'confirmed')])
        else:
            instances.append(instance)
        for instance in instances:
            wcapi = instance.connect_in_woo()
            sales_orders = self.search([('warehouse_id', '=', instance.warehouse_id.id), ('woo_order_id', '!=', False),
                                        ('woo_instance_id', '=', instance.id), ('updated_in_woo', '=', False)],
                                       order='date_order')
