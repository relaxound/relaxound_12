...
class stock_move(models.Model):
    ...
    def _search_picking_for_assignation(self, move_ids):
        cr = self.env.cr
        ids = self.ids
        uid = self._uid
...
#         return self.write(move_ids, {'picking_id': pick})
#
#     def action_payed(self):
#         """ Confirms stock move or put it in waiting if it's linked to another move.
#         @return: List of ids.
...
#                     todo.append(r.id)
#         if len(todo):
#             self.env('stock.move').action_payed(todo)
#
#         if todo_force_assign:
#             self.force_assign(todo_force_assign)
#         return True
...
#     #             return 'confirmed'
#     #         elif most_important_move.state == 'to_pay':
#     #             return 'to_pay'
#     #         else:
...
#     #                              store={
#     #                                  'stock.picking': (
#     #                                  lambda self, ids, ctx: ids, ['move_type', 'launch_pack_operations'], 20),
