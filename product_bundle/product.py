...
class product_item(models.Model):
    ...
    product_id = fields.Many2one('product.product', 'Bundl Product')
    ...
    def create(self, values):
        ...
        for variant in res_id.product_variant_ids:
            ...
            for pitem in res_id.pitem_ids:
                res = self.env['product.item'].create(
                    {'sequence': pitem.sequence, 'product_id': variant.id, 'item_id': pitem.item_id.id,
                     'uom_id': pitem.uom_id.id, 'qty_uom': pitem.qty_uom})
                items.append(res)
                pass
...
class product_product(models.Model):
    ...
    def write(self, vals):
        ...
        if self.product_tmpl_id.product_variant_count == 1:
            ...
            for pitem in self.pitem_ids:
                res = self.env['product.item'].create(
                    {'sequence': pitem.sequence, 'template_id': self.product_tmpl_id.id, 'item_id': pitem.item_id.id,
                     'uom_id': pitem.uom_id.id, 'qty_uom': pitem.qty_uom})
    ...
    def _product_move_in_bundle(self, product, context, domain_move_in_loc):
        ...
        domain_move_in += self._compute_quantities_dict() + [
    ...
    def _product_move_out_bundle(self, product, context, domain_move_out_loc):
        ...
        domain_move_out += self._compute_quantities_dict() + [
            ('state', 'not in', ('done', 'cancel', 'draft'))] + domain_products
