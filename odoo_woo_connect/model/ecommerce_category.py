...
from ..unit.product_ecommerce_category_exporter import WpEcommCategoryExport
...
class ProductCategory(models.Model):
    ...
    is_ecommerce=fields.Boolean('Is Ecommerce')
    ...
    backend_mapping = fields.One2many(comodel_name='wordpress.odoo.ecommerce.category',
                                      string='Category mapping',
                                      inverse_name='ecom_category_id',
                                      readonly=False,
                                      required=False,
                                      )
    ...
    def create(self, vals):
        ...
        ecom_category_id = super(ProductCategory, self).create(vals)
    ...
    @api.multi
    def sync_ecommerce_category(self):
    ...
    def export(self, backend):
        ...
        mapper = self.backend_mapping.search(
            [('backend_id', '=', backend.id), ('ecom_category_id', '=', self.id)], limit=1)
        method = 'category'
        arguments = [mapper.woo_id or None, self]

        export = WpEcommCategoryExport(backend)
        res = export.export_product_ecomm_category(method, arguments)
        if mapper and (res['status'] == 200 or res['status'] == 201):
            ...
            if res['data']['image']:
                ...
                image_id = None
            mapper.write({'ecom_category_id': self.id, 'backend_id': backend.id, 'woo_id': res[
            ...
            if res['data']['image']:
                ...
                image_id = None
            self.backend_mapping.create({'ecom_category_id': self.id, 'backend_id': backend.id, 'woo_id': res[
                                        'data']['id'], 'image_id': image_id})
        elif (res['status'] == 400 and res['data']['code'] == 'term_exists'):
            if 'resource_id' in res['data']['data'].keys():
                self.backend_mapping.create({'ecom_category_id': self.id, 'backend_id': backend.id, 'woo_id': res['data']['data']['resource_id']})
        return


class ProductEcommerceCategoryMapping(models.Model):

    """ Model to store woocommerce id for particular product category"""
    _name = 'wordpress.odoo.ecommerce.category'

    ecom_category_id = fields.Many2one(comodel_name='product.public.category',
                                  string='Product Ecommerce Category',
                                  ondelete='cascade',
                                  readonly=False,
                                  required=True,
                                  )
