...
class WpEcommCategoryExport(WpImportExport):
    ...
    def export_product_ecomm_category(self, method, arguments):
        ...
        if arguments[1].parent_id:
            mapper = arguments[1].parent_id.backend_mapping.search(
                [('backend_id', '=', self.backend.id), ('ecom_category_id', '=', arguments[1].parent_id.id)], limit=1)
            parent = mapper.woo_id or None
            if not parent:
                res = self.export_product_ecomm_category(
                    method, [None, arguments[1].parent_id])
                if res['status'] == 201 or res['status'] == 200:
                    arguments[1].parent_id.write({'slug': res['data']['slug'],'ecom_category_id':True})
                    if res['data']['image']:
                        image_id = res['data']['image']['id']
                    else:
                        image_id = None
                    if mapper:
                        mapper.write({'ecom_category_id': arguments[1].parent_id.id, 'backend_id': self.backend.id, 'woo_id': res[
                                     'data']['id'], 'image_id': image_id})
                    if mapper:
                        mapper.create({'ecom_category_id': arguments[
        ...
        mapper = arguments[1].backend_mapping.search(
            [('backend_id', '=', self.backend.id),
             ('ecom_category_id', '=', arguments[1].id)],
            limit=1)
